...
int player_turn(PLAYER* player_active, PLAYER* player_opponent, unsigned int board_size) {
    ...
    do {
        ...
        if (flag == VALID_HIT) { // Si el disparo fue un acierto
            ...
            // Si el disparo falló, muestra mensaje y contador para el próximo turno
            printf(RED_COLOR"\n\tFallaste! Es el turno del otro jugador.\n");

            // Contador de 10 segundos antes de cambiar de turno
            for (int i = 10; i > 0; i--) {
                printf("\r\tCambiando de turno en %d segundos...", i); // Muestra el tiempo restante
                fflush(stdout); // Asegura que el texto se muestra inmediatamente
            ...
            printf("\n"); // Salto de línea después del contador
        }
